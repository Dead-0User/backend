...
const sendSignupOTP = async (to, otp) => {
  const mailOptions = {
    ...
    to: to,
    subject: "Email Verification - QRMenu",
    html: `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>If you didn't sign up for QRMenu, please ignore this email.</p>
 ...
 */
const sendPasswordResetOTP = async (to, otp, restaurantName) => {
  const mailOptions = {
    from: `"QRMenu Support" <${process.env.EMAIL_USER}>`,
    to: to,
 ...
 * @param {string} to - Recipient email
 * @param {Object} credentials - Staff credentials object
 * @param {string} credentials.username - Staff username
 * @param {string} credentials.password - Staff password
 * @param {string} credentials.restaurantName - Restaurant name
 * @param {string} credentials.role - Staff role
 * @param {string} credentials.restaurantId - Restaurant ID
 */
    ...
    html: `
      ...
      <head>
        <style>
          ...
          .warning { color: #e53e3e; font-size: 14px; margin-top: 20px; }
          .footer { text-align: center; margin-top: 20px; font-size: 12px; color: #666; }
          .cta-button { display: inline-block; background-color: #667eea; color: white !important; 
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div style="text-align: center;">
              <a href="${process.env.FRONTEND_URL || "http://localhost:5173"}/staff/${role}/login?restaurantId=${restaurantId}" class="cta-button">
                Login to Dashboard
              </a>
              <p style="font-size: 12px; color: #666; margin-top: 10px;">
                Or copy this link: <br/>
                ${process.env.FRONTEND_URL || "http://localhost:5173"}/staff/${role}/login?restaurantId=${restaurantId}
              </p>
            </div>
            
            <p><strong>⚠️ Important:</strong> Please login and change your password upon your first access for security reasons.</p>
