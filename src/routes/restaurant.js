...
router.get("/current", authMiddleware, async (req, res) => {
  try {
    ...
    // Migration: Convert old fields to new licenseFields array if needed
    let licenseFields = restaurant.licenseFields || [];
    if (licenseFields.length === 0) {
      // Check if old format exists
      const hasOldFields = (restaurant.fssai || restaurant.fssaiLabel || restaurant.gstNo || restaurant.gstLabel);
      if (hasOldFields) {
        const tempFields = [];
        if (restaurant.fssaiLabel || restaurant.fssai) {
          tempFields.push({
            label: restaurant.fssaiLabel || "License 1",
            value: restaurant.fssai || "",
            order: 0
          });
        }
        if (restaurant.gstLabel || restaurant.gstNo) {
          tempFields.push({
            label: restaurant.gstLabel || "License 2",
            value: restaurant.gstNo || "",
            order: 1
          });
        }
        licenseFields = tempFields;
      }
    }
    ...
    res.json({
      ...
      restaurant: {
        ...
        fssaiLabel: restaurant.fssaiLabel || "",
        gstNo: restaurant.gstNo || "",
        gstLabel: restaurant.gstLabel || "",
...
router.put("/update", authMiddleware, upload.single("logo"), async (req, res) => {
  try {
    ...
    if (fssaiLabel !== undefined) restaurant.fssaiLabel = fssaiLabel;
    ...
    if (licenseFields !== undefined) {
      try {
        const parsedFields = typeof licenseFields === 'string' ? JSON.parse(licenseFields) : licenseFields;
        if (Array.isArray(parsedFields)) {
          restaurant.licenseFields = parsedFields.map((f, idx) => ({
            label: f.label || "",
            value: f.value || "",
        ...
        console.error("Error parsing licenseFields:", e);
      }
    }
